    ...
    <v-container>
        <v-row justify="center">
            <v-col cols="12" md="8">
                ...
                <v-card-text>
                    <v-data-table :headers="tableHeaders" :items="orderList" class="elecation-1" show-expand>
                        <template v-slot:[`item.actions`]="{ item }">
                            <v-btn color="red" v-if="isAdmin && item.orderStatus === 'ORDERED'" @click.stop="cancelOrder(item.id)"
                                size="small">
                                CANCEL
                            </v-btn>
                        </template>
